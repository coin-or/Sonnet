...
namespace SonnetTest
{
    ...
    public class SonnetTest
    {
        ...
        public void SonnetTestQuad1()
        {
            ...
            Assert(SonnetTest.EqualsString(obj.ToString(), "- 2 x1 - 6 x2 + 0.5 x1^2 + x2^2 - x1x2 + x1^2 - x1x2"));

            obj.Assemble();

            Assert(SonnetTest.EqualsString(obj.ToString(), "- 2 x1 - 6 x2 + 1.5 x1^2 - 2 x1x2 + x2^2"));

            obj.Add(6.0);
            Assert(SonnetTest.EqualsString(obj.ToString(), "- 2 x1 - 6 x2 + 1.5 x1^2 - 2 x1x2 + x2^2 + 6"));

            Expression expr = 6.0 - x2 + 1.5 * x2;
            expr = 2.0 * expr * x1;
            Assert(SonnetTest.EqualsString(expr.ToString(), "12 x1 - 2 x1x2 + 3 x1x2"));
            expr.Assemble();
            Assert(SonnetTest.EqualsString(expr.ToString(), "12 x1 + x1x2"));


            expr = 6.0 - x2 + 1.5 * x2;
            expr = x1 * expr * -1.0;
            Assert(SonnetTest.EqualsString(expr.ToString(), "- 6 x1 + x1x2 - 1.5 x1x2"));
            expr.Assemble();
            Assert(SonnetTest.EqualsString(expr.ToString(), "- 6 x1 - 0.5 x1x2"));

            expr = (6.0 - x2 + 1.5 * x2) * (6.0 - x2 + 1.5 * x2);
            expr.Assemble();
            Assert(SonnetTest.EqualsString(expr.ToString(), "6 x2 + 0.25 x2^2 + 36"));
            // 36 - 6 x2 + 9x2 - 6 x2 + x2^2 - 1.5x2^2 - 1.5x2^2 + 9 x2 + 2.25 x2^2
            // = 6 x2 + 0.25 x2^2 + 36

            expr = (6.0 - x1 + 1.5 * x2) * (6.0 - x1 + 1.5 * x2);
            expr.Assemble();
            Assert(SonnetTest.EqualsString(expr.ToString(), "- 12 x1 + 18 x2 + x1^2 - 3 x1x2 + 2.25 x2^2 + 36"));
            ...
            expr.Assemble();
            Assert(SonnetTest.EqualsString(expr.ToString(), "- 6 x1 + 9 x2^2 + 36"));
        ...
        public void SonnetTestQuad2()
        {
            ...
            Assert(MathExtension.CompareDouble(obj.Value, -74 / 9.0) == 0);
            
            Assert(MathExtension.CompareDouble(x1.Value, 2.0/3.0) == 0 &&
                MathExtension.CompareDouble(x2.Value, 4.0/3.0) == 0);

        }
        public void SonnetTestQuad3()
        {
            // Will cause NotSupportedException since MIQP is not supported.
            ...
            model.Objective = obj;
            model.ObjectiveSense = ObjectiveSense.Minimise;
            if (solver.OsiSolver is COIN.OsiCbcSolverInterface) ((OsiCbcSolverInterface)solver.OsiSolver).SetCbcSolverArgs("-branchAndBound");
